...
is_prepared_release_commit() {
  [[ "${PR_HEAD_SUBJECT:-}" =~ ^chore\(release\):\ v[0-9]+\.[0-9]+\.[0-9]+$ ]] || return 1
...
self_test() {
  ...
  PR_HEAD_SUBJECT='feat(cli): unprepared promotion head'
  PR_HEAD_BODY=''
  ...
  PR_HEAD_SUBJECT='chore(release): v1.2.3'
  ...
  is_prepared_release_head || fail "self-test expected the marked Release Bot commit to be prepared"
