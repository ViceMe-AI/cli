...
每个新对话在需要 CLI 时，先使用与宿主当前命令工具匹配的官方定位器：POSIX shell（包括 Windows Git Bash、MSYS 和 Cygwin）使用 `creator-tools/scripts/resolve-cli.sh`，PowerShell 使用 `resolve-cli.ps1`，不得为了定位从当前 shell 启动另一种 shell。优先保留 PATH 中现有的命令（包括 npm launcher）；否则查找 `VICEME_INSTALL_DIR` 或官方默认目录。POSIX 定位器在 Windows 上读取 `LOCALAPPDATA` 并通过 `cygpath`（可用时）输出当前 shell 可执行的绝对路径。定位器只输出完整路径，不安装、不更新、不推断安装方法、不读取凭据。调用方在所有独立工具调用中沿用带引号的完整路径，并按原流程执行目标命令；安装验收或调用流程要求时才检查 `version`。自定义目录没有加入 PATH 时，调用方须保留 `VICEME_INSTALL_DIR` 或安装返回的 destination。云端直读 Skill 的调用方可将同区域官方 S3 上的对应定位脚本下载到用户私有临时文件后运行，无需先安装 Skill。
