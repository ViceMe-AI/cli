...
2026-09-21 输入预算调整：服务端以 Gemini 3.8 Flash 的 1,048,576 输入 tokens 为基准，完整输入最多使用 80%，即 838,860 tokens。使用同模型 Count Tokens 对系统指令、私有 Skill 和当前完整任务统一计数；客户端无法读取私有正文，不自行估算。移除 prompt 8,000、facts 30 项/键 80/值 4,000 及合计 24,000 字符限制；facts 仍可省略，完整需求直接放 prompt。客户端和指导 HTTP 提交保留 16 MiB 传输保护。超预算不生成、不扣试用；本地 Agent 精简完整任务后换新键提交，不压缩、不原样重试。
