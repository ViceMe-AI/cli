...
Go 的 `PaymentPresentationGuide()` 与 `paymentPresentationHint()` 使用同源规则；Python Skill 运行包携带 `guides/host-presentation.md`。`use-a-skill` 与 `let-me-make-a-copy` 的 `references/host-presentation.md` 是带来源声明的自动生成分发副本，不手工维护。独立安装包必须自包含，不能引用开发仓库根目录。独立复制脚本也嵌入同一份指引。
