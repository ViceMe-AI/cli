...
7. 用户确认后连续完成确认与公开发布，返回公开链接；随后只问一次：“你这边还有没有升级版本或更高级的版本？用户免费用完这个版本后，平台会向他推荐你的高级版本，帮你把免费用户变成付费用户。”用 `AskUserQuestion` 给两个选项：“有，发布一个更高级的版本”“没有，就到这里”。用户选有就按正常发布流程处理新来源（见组合规则）；选没有则结束。向创作者解释追加条目时用“免费版/付费版/升级版/高级版”这类版本说法讲清商业价值：免费或低价版本让用户先用起来，高级版本承接愿意付费的用户。但版本说法不改变条目身份——“升级版”是同一作品页新增的独立 Skill，不覆盖、不替换已发布条目；改原条目还是新增条目仍按「发布意图与条目选择」区分。
...
每个修改或完成 Draft 的成功 CLI 结果都包含新 `presentation`。始终用 `presentation.openUrl` 打开稳定 HTML 作品预览页（`/{handle}/{slug}/preview`），不切换为 Markdown、一次性入口或 `fallbackUrl`。该地址长期有效、无需登录、页面只读；不要先要求用户在浏览器登录。打开链接的工具返回成功只证明入口已打开，未观察到页面内容时不能声称预览已正常显示。草稿更新通过 revision 轮询呈现。发布后的追加询问只遵循主线第 7 步的升级版提议，只问一次，不再重复追问。
