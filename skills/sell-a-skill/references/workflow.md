...
2. 按来源读取，仅私有 GitHub 按需授权；公开 GitHub、小红书和本地包直接进入下一步。
...
7. 用户确认后连续完成确认与公开发布，打开并返回公开链接；随后只问一次：“作品已经发布，默认页面也准备好了。接下来想做什么？”使用当前 Agent 的原生结构化单选工具给出三个中立选项：“自定义作品页”“发布一个更高级的版本”“先到这里”。不得给任何选项标记推荐或替用户选择；工具支持自定义输入时必须保留，工具不可用时给出相同的三个编号选项并说明用户也可以直接说想做什么。用户选“自定义作品页”时，把本次响应的准确公开链接和已确认 Merchant 直接交给 `$customize-your-work-page`，不让用户复制链接、重选作品或重新检查创作者资格；用户选高级版本时按正常发布流程处理新来源（见组合规则）；用户选结束时立即结束。自由文字已经明确表达其中一种意图时直接进入对应路径，只有确实无法判断时才追问一句。向创作者解释追加条目时用“免费版/付费版/升级版/高级版”这类版本说法讲清商业价值：免费或低价版本让用户先用起来，高级版本承接愿意付费的用户。但版本说法不改变条目身份——“升级版”是同一作品页新增的独立 Skill，不覆盖、不替换已发布条目；改原条目还是新增条目仍按「发布意图与条目选择」区分。
...
用户要「发布升级版/更高级的版本」「往组合里添加 skill」或「再发布一个」时，先确认新增条目及其内容。相同来源位置可能已有新的包内容，不能仅凭路径或标题断言只能更新；完全相同的已发布包也不能靠换 key 或名称绕过内容约束。意图不明确时先澄清，不得先创建 Publication 再靠报错或取消收场。
...
所选 `merchantAccountId` 是幂等发布意图的一部分，恢复时不能变化。恢复通常不需要 `--merchant`；传入时必须等于已保存商家。公开资料的变化不得选择、转移、暂停或恢复该商家；只有 OWNER membership 授予发布权限。
...
在 WorkBuddy 中用 Bash 后台启动并保存 `task_id`，用一次短时 `TaskOutput` 读取当前命令输出。公开来源没有授权步骤。只有私有仓库需要授权且命令实际输出一次性链接时，立即使用内置 `present_files` 在当前任务浏览器打开同一个链接，说明“请在右侧完成 GitHub 私有仓库授权，完成后我会自动继续。如果右侧不方便，也可以在外部浏览器打开下面这个链接：”，另起一行用 Markdown 链接格式输出当前命令实际返回的完整 `https://` 链接：`[打开 GitHub 授权页面](https://…)`。随后读取 `TaskOutput(task_id=<同一个任务>, timeout=180000)`；只要命令仍在运行，就继续读取同一个 `task_id`。不得要求用户回复“完成了”，不得再次启动发布或授权命令、轮询状态、`sleep` 或启动另一后台任务。CLI 完成有界等待后继续同一次发布，最终只返回一个 JSON 结果；明确失败或超时才停止。
...
包内没有可用图片候选时，不得直接开始生成或上传图片。先用 `AskUserQuestion` 问一次“需要为这个 Skill 准备封面和图库图片，你想怎么做？”，提供三个选项：让我自动生成（会消耗少量生图额度）、我自己提供图片、先跳过图片；不得把选择埋在长段落里，也不得要求用户手打长句。用户选择自动生成才调用生图工具，生成前仍要说明预计消耗；选择自己提供就等待用户给出图片文件；选择先跳过则继续文案与价格，最终确认发布前若仍缺必需媒体，再以同样方式提醒补齐一次。用户明确选择后 Agent 提供图片时运行 `publication asset upload <publication-id> --role cover|gallery --path <image> --candidate-only`，再通过 `publication suggest` 选择已验证 upload ID，来源保持 `AGENT`。多条媒体的上传命令必须逐条串行执行，不得并行发起多张上传（并行上传会基于同一快照争抢同一空闲槽位，导致部分上传报槽位冲突失败）。不带 `--candidate-only` 的默认选择形式只用于用户明确修改媒体，只写该媒体字段并标记来源 `USER`。两种形式都不排队执行平台分析，也不得仅因包内无图片就强迫 Agent 编造文案。
...
分析和必需媒体准备好后，取得权威预览，在索取更多输入前展示标题、中文简介、中文使用说明、封面和有序图库。在同一次交互中确认价格与试用次数以及希望修改的上架内容：review 返回的价格非空（含继承）时，先告知当前价格（免费要说“当前免费”）和当前试用设置再问「保持还是调整」；价格为空才是首次定价，定价为正数时同时问试用次数（默认建议 10 次，可选不开试用），免费条目没有试用。不得只问价格，也不得先收价格再展示文案和媒体。用户直接报出价格（例如「定价 9.9 元」）也不例外：必须先追问该价格的试用次数决定（含明确不开试用）并一并应用，才能进入最终确认；只应用价格而不问试用视为流程违规。用户确认保持或只给价格时保留所有已展示字段，价格或试用次数变化才用 `skill publish --resume <id> --price-minor <fen> --trial-use-limit <n>` 应用（只改试用次数可省略价格参数；关闭试用传 `--trial-use-limit 0`）；两者都保持不变无需重发。用户也要求修改时，把完整回答应用到同一 Draft，展示新预览，再取得并展示最终 review。价格只在最终公开确认前必需，私有媒体上传和分析不需要价格。

每个修改或完成 Draft 的成功 CLI 结果都包含新 `presentation`。始终保留 `presentation.fallbackUrl` 的稳定 HTML 作品预览页，不切换为 Markdown 或一次性入口。浏览器未登录时先在页面登录，再回到同一个 `/preview`；CLI 已登录不代表浏览器已登录。打开链接的工具返回成功只证明入口已打开，未观察到页面内容时不能声称预览已正常显示。草稿更新通过 revision 轮询呈现。发布后的追加询问只遵循主线第 7 步，只问一次，不再重复追问。
...
`trialUseLimit` 为 `null` 或省略表示不开试用；免费条目不得设置试用。
