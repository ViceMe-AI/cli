...
- Shop 的 Publication 与 ViceMe 作品页是发布状态和做同款入口的权威来源；原站按钮复制下方固定格式的创作邀请，地址使用发布结果 `result.workUrl` 对应的 `.md` 地址，不添加购买或安装步骤。
...
- 只使用本次成功发布或同一 Publication 的 `status` / `resume` 返回的 `result.workUrl` 与 `result.instruction`；作品展示地址就是该 URL，邀请中的参考地址使用该 URL 加 `.md`。中文复制内容固定为 `{creatorHandle} 邀请我一起创作「{title}」，参考 {workMarkdownUrl}，恢复口令 {replicaInstruction}，立刻开始吧～`；英文为 `{creatorHandle} invites me to co-create “{title}”. Refer to {workMarkdownUrl}; recovery code: {replicaInstruction}. Start now!`。口令不是秘密，只在公开介绍不可用时恢复已有权益，不能用于跳过报价创建新订单。例子中的作者、标题、地址与口令不能照抄。
- `creatorHandle` 和 `title` 使用本次终审已确认的真实作者 handle 和作品标题；恢复接入时从同一作品的公开信息补齐，不能猜名字或把 slug 当作标题。按原站语言选择中英文句式；把完整邀请作为普通文本写入剪贴板，嵌入源码时安全序列化动态文本，不拼接未经转义的 HTML 或脚本。缺少权威 URL 或 instruction 时读取同一发布状态，不猜地址或口令、不重新发布。邀请不包含 localhost、target 包装地址、支付链接或额外执行步骤。
...
- 发布成功后原位启用正式“复制文案”功能，保持创作者已确认的位置、尺寸和样式，点击复制上述包含作者、作品标题和 Markdown 地址的完整邀请。成功时在入口上方短暂显示 `口令已复制！` 和 `粘贴到你的 Agent，即可生成同款，并继续修改成你的版本。`，失败时在同处提示重试；不能直接打开窗口或调用 CLI。不引入平台浏览器 SDK、loader、认证、价格或支付接口，不复制平台宿主实现到项目中。
