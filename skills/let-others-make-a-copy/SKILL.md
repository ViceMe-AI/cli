...
- 只使用本次成功发布或同一 Publication 的 `status` / `resume` 返回的 `result.workUrl`；作品展示地址就是该 URL，邀请中的参考地址使用该 URL 加 `.md`。中文复制内容固定为 `{creatorHandle} 邀请我一起创作「{title}」，参考 {workMarkdownUrl} 立刻开始吧～`；英文为 `{creatorHandle} invites me to co-create “{title}”. Refer to {workMarkdownUrl} and start now!`。例如 `yexu 邀请我一起创作「你的性格測驗」，参考 https://viceme.cn/yexu/personality-test.md 立刻开始吧～`，例子中的作者、标题与地址不能照抄。
- `creatorHandle` 和 `title` 使用本次终审已确认的真实作者 handle 和作品标题；恢复接入时从同一作品的公开信息补齐，不能猜名字或把 slug 当作标题。按原站语言选择中英文句式；把完整邀请作为普通文本写入剪贴板，嵌入源码时安全序列化动态文本，不拼接未经转义的 HTML 或脚本。缺少权威 URL 时读取同一发布状态，不猜地址、不重新发布。邀请不包含 localhost、target 包装地址、Replica instruction、支付链接或额外执行步骤。
