...
- 先复用用户已经明确提供的价格、标题、摘要和目标；不重复询问。更新作品时未要求改价就沿用现价。不为了展示选项而增加推荐文案、推广素材或是否留言等弹窗。
- 只有当前流程必需的信息缺失或需要创作者确认效果、发布时才提问。WorkBuddy 必须优先调用当前会话的 `AskUserQuestion`；工具未直接暴露时，使用宿主工具搜索查找并按实际 schema 调用，不只在正文写“请选择”。不可用或调用失败时明确说明，退回简短编号选项，不声称已弹窗。其他宿主优先使用其原生结构化提问工具。
...
- 效果确认和最终发布确认同样使用结构化提问工具，各自只确认尚未确认的内容，不重复问价格。最终发布仍绑定 CLI 返回的完整 review 和原始 confirmCommand，不把减少弹窗解释为自动批准发布。
...
1. 先按下方“原站做同款入口”在实际项目加入预览态按钮，由创作者确认页面和按钮效果即可；已经确认时直接继续，不要求启动或保活本地服务。只读取定位入口、样式和启动方式所需的文件；纯 HTML 可用静态服务，不补造 package.json 或改名 index.html。ZIP 输入先安全解压到独立的持久交付目录，后续修改、预览及首次发布统一使用该目录，不覆盖原 ZIP；已有 Publication 则恢复原请求，不更换绑定。
   ...
   简短告知“做同款按钮已准备好，请看看样式并点击试试；按钮和文案都可以按你的想法修改，保留作品链接即可。确认后我再发布。”尚未确认时等待创作者明确确认；内容未变且已有确认时不重复要求展示或确认。有样式问题只按创作者要求调整按钮并主动重新展示，取得确认后再更新线上作品；不重复定价。不做 HTTP 连通性探测、自动浏览器验收或截图检查，也不要求运行 `replica preview`。如果为展示临时启动了服务，只清理自己启动的服务；服务是否仍运行不影响发布。
...
3. 发布时邀请作者补充作品介绍：能做什么、适合谁、核心亮点、可以如何改成自己的作品；已有信息直接整理，不重复询问。将作者确认的信息压缩到现有 `--summary`（最多 500 字），在最终发布审核中展示，不能虚构使用量或评论，不新增不存在的发布参数。更详细说明沿用作品现有正文编辑能力，资料不足不阻塞源码发布。
...
6. 只依据 `REPLICA_PUBLICATION_CONFIRMATION_REQUIRED` 的完整 `review` 做一次最终不可变发布确认。Agent 核对全部身份、目标、价格、源码排除项、SOURCE/PAGE 摘要及降级授权；本次标记了入口的源码文件必须在 `sourceArchive.excludedPaths` 中有 `reason: creator-entry-blocks` 回执，缺少时按能力恢复流程停止并更新 CLI，不能确认上传。向创作者只展示有助决策的简短摘要：哪件作品、创建还是更新、最终价格、作品预览，以及“将发布此页面并交付其源码”。确实影响交付的排除项或未验证范围必须说明；不展示 ID、短码、对象 Key、SHA-256、内部状态枚举或恢复命令。用户已接受且未变化的信息直接列为摘要，不再次逐项询问。确认版本有效期为三十分钟，字段变化必须刷新终审并确认变化。
...
- 预览态保持现有按钮外观，默认点击仅提示“预览效果，发布后可复制”，不放假口令、不写剪贴板。通过本次预览 URL 的 `creatorPreview` 标识在 WorkBuddy 右侧容器中显示，不能用 `window.top !== window` 或仅识别 localhost 的判断把主动预览隐藏。普通托管 iframe 仍隐藏原站入口，托管页面的做同款功能由平台宿主提供。初次发布前的预览按钮不在普通公开地址显示。
- 首次接入时，把按钮、弹窗、复制及自动弹出逻辑、专属样式和相关 import/挂载点全部放进成对的 `VICEME_CREATOR_ENTRY_BEGIN` / `VICEME_CREATOR_ENTRY_END` 独占行注释中。已有未标记入口先按实际代码补齐边界，不能仅改成隐藏。CLI 在冻结目录或 ZIP 的交付副本时移除这些块；创作者原站不变，用户做同款后重新构建也不会出现原作者按钮。不扩展清理原作者的赞赏、弹幕、署名或许可证。
- 每次自定义调整都主动展示最新预览并取得创作者确认，再通过同一作品的发布/更新流程同步线上；不擅自更新已经上线的作品，不重问现有价格。再次发布保留旧正式入口可用，仅在本地预览展示调整；失败或取消不能破坏旧线上版本。
- 只有同一 Publication 达到 `PUBLISHED` 或 `PUBLISHED_DEGRADED` 后，才将原项目入口启用为正式态并填入已确认的文案与权威地址，保持已确认的样式与触发方式。失败、未知或 `PROCESSING` 不启用。复制功能原样写入创作者确认的邀请，复制失败时提示重试或提供可选原文供手动复制，不报告复制成功。
...
2. **复制口令**：提供创作者确认的邀请文案及权威作品地址，原站按钮保持现有样式和交互，除非创作者已要求自定义。
3. **经营数据**：“想了解作品卖出了多少份、有多少收入，直接问我就可以。”不用让创作者自己找 MD 或后台页面。
4. **服务号通知**：只有平台确认该创作者已具备或可配置通知能力时，说明真实状态与官方设置入口。未接入、未绑定或状态未知时不能承诺已开启，不猜服务号或二维码，不增加无关弹窗。
...
未登录时引导登录同一创作者账号后重试；无权限时停止，不能换身份或进入自己的购买流程。接口成功但 MD 未包含平台 OWNER 经营区时，报告当前未获取经营数据，不推算。网络或读取失败时说明暂时无法查询，不回退旧数字。命令缺失时只按 CLI 能力恢复流程更新；不让 Agent 自行读取配置中的登录凭据。
