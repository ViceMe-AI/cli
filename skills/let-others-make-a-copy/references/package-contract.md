...
源码 ZIP 不得包含有效 `VICEME-REPLICA:VMR-...` instruction、真实 `buyerEntry`，也不得包含 ViceMe 官方做同款挂件、弹窗或浏览器接入实现。ViceMe 托管 PAGE 保留自定义入口，通过宿主握手选择自定义组件或平台默认入口。创作者原站入口按 [creator-entry.md](creator-entry.md) 标记完整边界，CLI 在冻结交付副本时移除按钮、口令弹窗和相关代码；预览态也不进入买家源码。CLI 在移除前仍检查敏感和平台控制内容，标记不能绕过安全策略。原站入口和署名、许可证、赞赏、弹幕保持原样；发布成功后仅在原项目启用真实口令，不改写已经确认的制品。
