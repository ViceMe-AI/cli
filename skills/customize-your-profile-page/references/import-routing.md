...
平台将 HTML 和资源放在按 release 隔离的子目录，作者页在 sandbox iframe 中加载
...
- React、Vite 和 Next 可以是 Agent 本地制作或导入时的源码来源；最终上传物只能是已构建的静态 ZIP，内含根级 `viceme-page.json`、HTML 入口和所有被页面引用的静态资源。
- 不得上传 React、Vite 或 Next 的原始源码，也不得把 `src/`、Node 运行时、SSR、私有 API 或未构建的 `package.json` 项目当作可直接托管的页面。
- 构建失败时，要求用户提供可构建源码或已导出的网页包；不得上传原始源码碰运气，不新增云端构建器，也不以扫描到某个框架文件就直接拒绝。
- 用户仅提供网页 URL 或截图时，它们可用于内容与视觉的参考重制，不能作为 1:1 导入的可运行源码；单张图片可在用户确认公开后作为头像或封面素材。
...
继续 `viceme profile page upload/status/publish` 流程，不增加新接口、新压缩格式或上传限制。
打开 `profile page describe` 返回的准确地址，再检查相同交互；个人主页发布不等待创作者资格审核。
...
不要自动申请创作者资格、重登录、循环发布或自行回滚别人的页面。
