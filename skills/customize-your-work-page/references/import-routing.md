...
平台将 HTML 和资源放在按 release 隔离的子目录，作品页在 sandbox iframe 中加载
该 HTML。iframe 的 pathname 是资产入口路径，不是 `/`，也不是外层作品页地址。
资产服务按真实文件路径读取，不提供 SPA history fallback。登录和上传协议不因此改变。

## 导入准备

- 有源码：复用项目依赖和构建流程，在源码中修正路径后重新 build。托管适配本身只动路由和构建
  配置；视觉只按用户已经提出的要求修改。
...
AI 自己检查首页、至少一个内部跳转、浏览器前进/后退及 iframe 文档刷新；外层作品页刷新会重新
...
继续作品页的 preview/publish 流程，不增加新接口、新压缩格式或上传限制。
打开平台返回的准确地址，再检查相同交互。
