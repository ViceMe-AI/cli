...
9. steps 确认通过后 publication 换发 `confirm_publish` action。从最新 `job wait` / `job get` 的 `next_action.payload.preview_share_url` 读取仅当前创作者可访问的 `/p/{code}` 私有 Candidate 预览链接；它不是正式 `/v/{code}` 分享链接。用 `viceme job preview <id>` 读取 `data.preview` 中当前精确 Candidate 的公开摘要、candidate digest、payload digest、public-summary digest、分类定义与当前成功结果 manifest，并向用户展示标题、描述、来源作者、输入方式、使用方式、输出说明、示例和警告。用户提出自然语言修改时，将用户原话作为子进程的标准输入，运行 `viceme job edit <id> --candidate-digest <当前摘要里的 digest> --request-stdin`。只能通过 Host 的 subprocess stdin 通道传递原文；不得把原文拼入命令字符串、argv、环境变量或 shell 管道。CLI 会原样读取完整输入（包括换行和 shell 元字符）。相同请求的网络重试被服务端幂等去重；409 `candidate_changed` 说明摘要已过期，重新 `job get` / `job preview` 取新 digest 再问用户。不要引导用户去页面编辑器，也不要自己构造 JSON Patch。

### 私有 Candidate 预览与发布确认

10. 若 `data.preview.classification_schema` 有多个可选分类，先询问本次要试跑的 pre-run 分类；用重复参数 `viceme job preview <id> --classification key=value [--classification key=value]` 获取带不透明 `selection_receipt` 的新 `preview_share_url`，再把该 URL 交给用户。用户切换分类时重新执行同一命令；服务端会换发新会话代际，不能改写已开始的旧会话。in-run 分类不在 CLI 预选，由 Runner 在 `finish_run.resolved_dimensions` 中按冻结的 `resolution_key` 回报。若只有一个分类/一个结果，不展示子技能选择，直接使用默认 URL。每个需要挂载的分类至少完成一次 outcome=success 且有真实产出的普通运行；失败、partial、无产出的 success 均不挂载。无需穷举所有组合，后续再次发布可用最新成功结果更新同一分类。分享页没有“接受结果”或“确认发布”专用控件，CLI 也不提供独立 `job run`、`job run-get` 或 `job accept`。
11. 用户完成所需分类并核对结果后，重新运行 `viceme job preview <id>`，读取最新 `data.preview.result_manifest_digest` 与 `mounted_result_summary`，再明确确认或取消。最终确认使用 `viceme job resume <publication-id> --action-id … --expected-payload-digest … --expected-release-candidate-digest … --expected-public-summary-digest … --expected-result-manifest-digest … --decision confirm`；取消不需要 result-manifest digest。若当前 Preview 尚无成功且有产出的运行，确认返回 409 `preview_share_run_required`；若试跑结果在确认前变化，返回 409 `result_manifest_stale`，重新读取 preview 后再请用户确认。确认只返回 `release_authorized` receipt；随后有界运行 `viceme job wait <id> --timeout 60s`，直到 `share_published` 或其他终态，再返回正式 `data.result.share_url`、`data.result.published_noop` 和 warnings。不要要求正式 `/v/{code}` 与私有 `/p/{code}` URL 相同；发布后旧预览地址会重定向到正式地址。

预览输入、消息、会话、Runner 历史和未被最终 manifest 引用的临时文件会在确认、取消、过期、被新 Candidate 取代或 Release 提交后清理，不进入公开浏览量、使用量、作品或历史。只有最终 manifest 引用的成功结果 bundle 会冻结到主 Skill；平台内部产物提升为只读引用并在分享页按访问时重新签名，外部链接仅按结构化元数据保留，不保证第三方资源永久有效。
