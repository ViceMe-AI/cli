...
CLI 从当前可执行文件的内嵌 Skill FS 读取模板，不依赖用户当前安装目录的可变脚本。运行库暴露 `ViceMeMiniGameCommerce`；受管配置通过 `createMiniGameRuntime(configuration, createMiniGamePlatform(window))` 创建 `window.ViceMeMiniGame`。宿主不能再创建另一个实例，也不能重复加载配置。

`createMiniGamePlatform(window)` 按宿主选择平台：有 `window.xhs` 或小红书 UA 标记时使用小红书原生存储与相册，能力不足、版本不支持或接口读写失败均直接报错，不回退浏览器。普通浏览器使用独立 IndexedDB 持久化安装与权益，不与小红书存储合并，不静默降级到内存或 localStorage；只有确认记录不存在才可创建安装身份，读取失败不能生成替代身份。

普通浏览器的付款卡沿用运行库绘制，通过可关闭的原生对话框展示，支持 ESC、焦点恢复及手机布局；PNG 下载和打开结算页均需玩家明确点击，不自动弹窗或付款。小红书仍保存付款卡到相册。宿主只调用同一套接口，不另写平台探测、存储或付款卡实现。
...
`checkoutOrigin` 必须是规范化 origin，不带路径、查询、片段、用户信息或尾斜杠。正式部署要求 HTTPS；本地开发服务允许 `http://localhost`、`http://127.0.0.1`、`http://[::1]`，可带端口，不支持其他公网或局域网 HTTP。该例外由 API 的部署环境控制，不由作品的 sandbox/production 选择决定；只使用 manifest 返回的地址，不为绕过校验改写受管配置或切换到另一站。
...
动态码绑定安装、作品、环境、道具和时间窗口，不能跨安装兑换或换绑。同一安装可重新取码，正常重启可恢复已经兑换的本地权益；小红书与普通浏览器、不同浏览器或不同网页 origin（协议、域名、端口）是不同安装，不共享权益。清数据、卸载重装或换设备会丢失绑定权益，禁止提供安装 ID 编辑、导入备份或“恢复旧安装”入口。购买/兑换区域需说明该代价。游戏内容预置，共享密钥内置后可被提取并伪造动态码，不是强 DRM，不能保证防止内容提取或修改。

受管文件、项目 API 站点、作品、商家与环境通过管理状态绑定。中断可重跑 integrate，只接受已记录的旧/目标哈希；人工修改、错绑及未知数据停止覆盖。`check` 不修改项目，也不会执行游戏脚本；通过不代表运行时或小红书平台验收通过。

## 真实验收边界

普通网页可直接用原项目的预览方式验收真实 IndexedDB 持久化、付款卡展示/PNG 下载、兑换与刷新恢复，无需先完成小红书打包。必须加载受管的真实运行库，使用当前环境、当前安装和对应道具的有效动态码；不得 mock `initialize`、`redeemCode`、`isUnlocked` 或用本地解锁开关代替。没有有效动态码或支付条件时，如实报告未验证部分，不能把静态检查、模拟 UI 或 sandbox 结果当作真实支付链路通过。

用户要发布小红书时，再交回小红书上传页的当前官方口令与对应 Skill 完成兼容检查和打包；浏览器通过不代表小红书真机通过。此接入流程不替用户打包、上传或发布。
