...
满足条件后，在当前 Agent 工作区说明作品已可预览，再按“用户交互”提供适用的扫码登录动作和“暂时跳过”。只依据本次流程已知状态或权威返回的登录入口展示：已有兼容 CLI 且已知未登录时，用户选择登录后复用 `creator-tools` 的登录流程；已登录不重复提示。Python 路径没有已知登录状态或可用登录入口时不臆造，不为展示提示额外定位 CLI、探测账号或要求安装 CLI。用户跳过后继续已授权的创作，不重复弹出提示，登录不是预览、修改或部署的前置条件。
