...
2. 在对话中说明两条入口，右侧不承担选择弹窗：
   - **查看模板**：先解释模板会提供已经校准的布局、信息层级和本机可预览的起点，再列出所有已登记、可打开且获授权的模板。每项必须给名称、适用场景、简述和实际可打开的本地样例入口；不得虚构模板名称、查看链接或授权。当前 bundle 只有 [Bonjour Card](templates/bonjour-card/README.md)，所以只展示这一项，不能声称有 5–10 个模板。用户选中后，完整阅读其 `README.md` 和 `DESIGN.md`，复制整个项目到用户工作目录，并先 build、打开本机样例。禁止从空白页面重新生成，禁止切换技术栈，也禁止把模板当作可以自由发挥的灵感；除非用户看过本机预览后明确要求某项视觉改动，不得修改既有组件结构、class 名或 `src/styles.css`。
...
3. 模板或导入方式确定后，一次性邀请用户提供已有资料，不要一项一项循环追问。按四类收集：
...
4. 有源码时运行项目自己的 build；使用 [本地嵌套路径预览](references/import-routing.md#本地预览) 打开本机 HTML 预览，
