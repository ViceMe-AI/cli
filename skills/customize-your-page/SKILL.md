...
用户说“创建个人名片”“完善个人名片”“我的创作者主页”或同义的**自己的个人名片**时，
...
3. 选定真实模板后，先一次性邀请用户提供已有资料；不得先询问目标目录或额外的本机写入确认，也不要一项一项循环追问。按四类收集：
   ...
   基于已收到资料先输出“可直接使用 / 待确认公开 / 缺失”三栏清单，再一次性集中追问缺失的必要项。名称和头像优先使用 `qualification.user` 及平台 `context.read`，已有简介直接复用。本期不支持视频；不添加文字、图片墙、App、公众号、高光、教育、工作经历、获奖等 Block。
4. 必要资料补齐后，使用当前用户工作区中以模板 ID 命名的子目录创建本机项目；只有发现同名的非本次项目而可能覆盖时才询问用户。不得把目录选择变成普通用户的必答题。真实模板有源码时运行项目自己的 build；使用 [本地嵌套路径预览](references/import-routing.md#本地预览) 打开本机 HTML 预览，
