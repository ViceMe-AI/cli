    ...
    for base in target_roots(agent):
        ...
        for name in sorted(os.listdir(base)):
            ...
            try:
                ...
                if (manifest.get("schemaVersion") not in (1, 2) or manifest.get("productId") != product_id
...
def run_cloud_task(market, product_id, value, wait=60):
    ...
    credential = require_trial_state(market, product_id)
    filename = os.path.join(cloud_task_directory(market, product_id), value["requestKey"] + ".json")
    record = {"schemaVersion": 1, "apiBaseUrl": API_ORIGIN[market].rstrip("/"), "market": market,
              "principal": "trial:" + credential["installId"], "input": value, "paymentRequired": False}
    with ProductLock(product_id):
        try:
            with open(filename, encoding="utf-8") as handle:
    ...
    try:
        result = cloud_result(api_request(market, "POST", "/v1/skill-cloud/trial/requests", {**value, "installId": credential["installId"], "secret": credential["secret"]}, canonical_errors=True))
    ...
    while result["status"] in ("QUEUED", "RUNNING") and time.monotonic() < deadline:
        ...
        result = cloud_result(api_request(market, "POST", "/v1/skill-cloud/trial/requests/%s/read" % result["requestId"], body, canonical_errors=True), result)
...
def parse_args(argv):
    ...
    parser.add_argument("command", choices=["ready", "install", "use", "status", "purchase", "cloud"], help="ready=只读确认本机安装,install=安装,use=申请一次试用,status=查询余量不扣次,purchase=付款并转正")
    parser.add_argument("--wait", type=int, default=0, help="展示二维码或官方支付链接后有界等待支付的秒数(0–600)")
    parser.add_argument("--input", help="cloud 任务 JSON 文件，同 UUID requestKey 重试必须保持输入不变")
