...
def purchase_required_message():
    return ("Skill 已安装，需要购买后使用。同一轮立即运行 runtimePath 指向的本地脚本 purchase --wait 0，"
            "并执行已装 SKILL.md 的「使用前必读」：先写白话开场白，再把对话内二维码、可点击支付链接和宿主支付页写给用户看，"
...
def payment_display_instructions(hosted=False):
    ...
    instructions = (
        "回复正文必须先有白话开场白，再展示支付入口：说明现在为什么要付费，金额只用返回字段；"
        "无试用入口用 SKILL.md frontmatter 的名称和简介，并说明没有试用、付费后才能开始；"
...
def purchase_entry_skill_markdown(installed_name, description, product_id, market):
    ...
    return (
        ...
        "不要走账号购买或订阅。"
        "给用户看的回复必须先有白话开场白，再展示支付入口：用本入口 frontmatter 的名称和简介说明这件能做什么、没有试用、需要先付费才能开始；金额只用返回字段。"
        ...
        "仅当结果 `kind=owned`、`allowed=true` 时，重新读取返回的 skillPath，继续原任务。"
