...
无试用付费版使用作品页的 Python install 入口；无需 CLI 或登录。脚本首次调用先持久化当前商品和市场的购买凭证，再创建或恢复订单，返回与试用转购买相同的支付展示。不要创建「零次试用」，也不要安装试用上手卡。

首次安装尚未写入商品目录时，使用响应中的 runtimePath 作为后续命令的绝对路径，执行 `python3 '<runtimePath>' purchase --product <product-id> --market <market> --wait 60`（Windows 用 py）。先按下方「通用支付展示」出示二维码和官方收银台链接，再有界等待。付款确认后由同一脚本校验权益、下载正式包并安装，读取实际 SKILL.md 后继续原任务。没有原任务才介绍用法。
