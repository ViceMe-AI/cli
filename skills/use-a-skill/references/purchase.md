...
安装返回 PURCHASE_REQUIRED，或用户直接提供 kind=purchase 的渠道包时，按包内 SKILL.md 打开 references/entry.md，执行其中的「使用前必读」，运行本地脚本 `python3 '<runtimePath>' purchase --product <product-id> --market <market> --wait 0`（Windows 用 py）。命令会保存本机购买身份并创建或恢复订单；已有已付款订单时，会校验权益并继续安装。执行遵循当前宿主的授权要求，已有授权继续沿用。
