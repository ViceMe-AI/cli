...
description: 为创作者公开网站发布或复用 Website Work，并同时接入 ViceMe 托管弹幕和赞赏。先复用 $become-a-creator 检查资格，再完成域名验证、平台配置和宿主嵌入。
---

# 给网站接入 ViceMe 弹幕和赞赏

实现一个真实的创作者网站和一段 ViceMe 嵌入脚本。创作者登录、申请和商家选择全部交给
`$become-a-creator`。修改前完整阅读
...
7. 读取该 Work 的 `sdk-access`。缺失时同时创建 `danmaku` 和 `tip` feature；已存在时按当前
...
- 已有未验证域名时复用并重新部署当前 challenge；已有已验证 Work 时复用其 Work、
  `workKey` 与 Widget 应用。
- 登录、资格或申请问题交回 `$become-a-creator`，不得在本 Skill 内重跑登录。
