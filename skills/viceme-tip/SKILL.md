...
1. Run `viceme profile list` and capture the active Profile, its `apiBaseUrl`, and its `webBaseUrl` without switching it. Pin every later CLI command to that Profile. A regional default Profile resolves its official Web origin automatically; a custom API Profile must carry an explicit `webBaseUrl`. If a custom Profile omits it, stop and ask the user to recreate that Profile with `--web-base-url <origin>`. The agent must not guess a Web origin and must not fall back to a production origin. Completion: one Profile, region, API base URL, and Web base URL are explicit.
...
4. Search that HTML for an existing ViceMe tip script and `data-creator-app-id`. Treat the work as bound only when the Creator App ID belongs to the user and the exact public hostname is verified in Creator Center. A script tag alone is not proof of binding.
5. When the work is not bound, open `<webBaseUrl>/<locale>/me/creator-center/apps` using the exact `webBaseUrl` reported by the selected Profile, then fetch the page before interacting. Do not substitute a memorized, regional, or production hostname. If the platform returns 404 or does not expose Creator Apps, stop and report that this exact selected environment has not deployed the binding capability; do not fabricate a work or URL. Otherwise guide the user to create an External work, add only the target hostname, and copy the exact verification URL/path and token displayed by Creator Center. Serve that token verbatim at the displayed public path, deploy it, fetch the HTTPS URL, then ask the user to click Verify. Completion: Creator Center marks the exact hostname verified and exposes its embed snippet.
6. Read the generated embed snippet from Creator Center. Use its complete script URL and Creator App ID exactly; never derive or invent either value. Insert one idempotent script tag before `</body>`:
