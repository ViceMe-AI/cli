...
原始请求同时包含赞赏，且接入流程尚未完成精确 SDK 发布预检时，必须先加载 `viceme-tip`。预检必须早于首次 `skill publish` 以及任何 Listing、Publication、Work、SDK access 或宿主页写入；失败就停止，不创建发布状态。若 `viceme-tip` 已完成预检并因缺少合格 Work 回到这里，不得重复预检。Skill 公开发布后，再返回 `viceme-tip` 对新 Work 完成接入。
