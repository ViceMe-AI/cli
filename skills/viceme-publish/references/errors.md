...
- `CREATOR_MONTHLY_PRICE_REQUIRED` or `requiresCreatorMonthlyPrice: true`: this is the creator's first upgraded Skill without an active monthly price. Ask the creator for one monthly CNY price in fen, then resume the same Publication with `--creator-monthly-price-cents <fen>`. This shared price unlocks every upgraded Skill by that creator. Never ask for a Skill price or a 1/3/6/12-month option.
- `SKILL_PUBLICATION_LEGACY_ACCESS_MODE`: the existing publication uses the retired one-time-purchase mode and cannot continue through this Agent-Native flow. Do not convert it silently.
...
- `PUBLICATION_RECOVERY_PERMISSION_REQUIRED`: the command process cannot write the local ViceMe publication recovery directory. Request filesystem permission for the exact same command and retry it with the same source and access mode. Do not delete lock files and do not create another publication.
