...
- 收到明确发布请求后，第一条用户可见回复只能是逐字的“我先检查登录和创作者资格。”，不得在这句话里补充 GitHub、仓库、渠道等内容。随后立即调用 `$viceme-creator-onboarding`；不得先建立计划、读取无关文件、检查环境或运行帮助命令。
...
- 发布任务的任何阶段都不得用 WebFetch、浏览器或 `curl` 读取 GitHub 仓库、SKILL.md 或包内容；包内信息一律以 `publication review` 返回的已验证数据为准。
