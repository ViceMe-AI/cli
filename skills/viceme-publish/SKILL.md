...
4. Show the final package digest, discovered image candidates, title, summary, and price. Ask permission to upload.
5. Run the same command without `--dry-run`.
6. Poll with `viceme publication get <id>` while analysis is `PENDING`. Analysis only suggests media; do not present it as a user decision.
7. Run `viceme publication review <id>` and show the exact price, cover, ordered gallery, and `reviewDigest`.
...
9. Ask the user to explicitly confirm the displayed price, cover, and gallery.
