---
name: viceme-publish
description: 路由并完成当前可用的 ViceMe 创作者端发布。适用于发布可下载 Skill、交易型服务与商品、创作者网站，以及为网站配置弹幕、打赏、关注或付费解锁；创作者资格由 $viceme-creator-onboarding 负责。
---
...
- 网站——创作者自有网站：完整阅读 [website-workflow.md](references/website-workflow.md)，创建或复用已验证的 Website Work，并在同一发布流程中配置弹幕、打赏、关注和付费解锁。网站本身不创建 Product；每个付费解锁功能由平台自动创建独立的非公开 Product。平台配置完成后，只有弹幕时交给 `$viceme-danmaku`，包含打赏时统一交给 `$viceme-tip` 维护共享 loader，关注与付费门控交给 `$viceme-access`；这些接入 Skills 不得反向发布或修改平台资源。
