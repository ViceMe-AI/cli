...
2. 未登录或缺少上述任一权限时，先说“需要重新登录，我现在为你打开登录页面。”，然后只启动一个等待式 `viceme auth login`。在 WorkBuddy 中，从进程首次输出读取完整登录链接，立即用内置 `present_files` 在当前任务浏览器打开；`cwd` 必须使用当前任务真实工作目录，不得编造路径或用户名。页面打开后立即说“请在右侧完成登录，完成后我会自动继续。”，再继续等待同一个进程返回。允许为读取同一个等待式登录进程使用 Bash 后台执行和 `TaskOutput`，但这不代表可以创建用户可见任务清单。不得启动第二次登录，也不得要求用户回复“已经登录”。登录成功后说“登录完成，我继续确认创作者资格。”，再运行一次 `viceme auth status`，确认还是同一用户且四项权限齐全；仍不满足就停止并用白话说明登录没有取得所需权限。
