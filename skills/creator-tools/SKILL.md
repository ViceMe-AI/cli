...
`viceme auth login` 会等待授权，并在轮询前输出一次性完整链接。在 WorkBuddy 中先说“需要重新登录。请点击下面的链接继续，当前右侧页面会保持不变。”，然后严格执行下面的固定顺序：

1. 用 Bash 后台启动一次 `viceme auth login`，保存它返回的 `task_id`。
2. 用一次短时 `TaskOutput` 读取完整登录链接。
3. 不自动调用 `present_files` 或任何浏览器打开工具。另起一行用 Markdown 链接格式输出第 2 步当前命令实际返回的完整链接：`[打开登录页面](https://…)`，其中 `https://…` 替换为该完整链接；当前右侧页面保持不变，用户点击链接后再由宿主按自身行为打开。不要直接贴裸链接，不得重建、缩短或复用旧链接。
4. 紧接链接说：“点击下面的「打开登录页面」继续；当前页面会保持不变。完成后我会自动继续。”
5. 提示发送后必须立刻调用 `TaskOutput(task_id=<同一个任务>, timeout=180000)`。发送提示不等于继续等待，用户点击链接也不代表登录完成。
