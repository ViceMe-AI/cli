...
`"permission_required"` 则表示自动更新在修改安装版本前被权限检查阻止；`code` 和
`hint` 说明需要的宿主授权，`data` 仍是原命令的执行结果，不能视为更新成功。
...
npm 安装会在创建更新事务前，通过 Node 检查实际的 npm 缓存、全局包及命令入口目录，
保留宿主原有的文件权限代理。预检发现权限不足时停止更新，原有完整版本仍可使用。
预检之后权限仍可能变化；一旦已经尝试安装，失败时会保留恢复记录并停止业务操作。
