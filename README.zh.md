...
首次安装会创建 `default` Profile。设备登录凭证按 Profile 与规范化 API origin 隔离；在 macOS 上，它们保存在私有的 AES-256-GCM 加密文件中，加密主密钥通常只保存在系统 Keychain；其他平台继续使用原生凭证管理器。Profile 文件只保存 endpoint 与选择配置，永远不保存 access token。
...
本地 endpoint 联调可创建只含地址覆盖的独立 Profile：
...
正常 `viceme auth login` 永远不会向 Profile 写入凭证；CLI 不提供保存 publication authorization credential 的 flag 或 Profile 字段。

可以用 `VICEME_CLI_CONFIG_DIR` 覆盖配置根目录，`VICEME_API_BASE_URL` 仍可作为单进程 endpoint 覆盖。受信任 launcher 注入的 `VICEME_ACCESS_TOKEN` 必须使用 `vpa1.<audience>.<secret>`，且只在当前进程有效：`cn-prod` 只能访问 `https://api.viceme.cn`，`global-prod` 只能访问 `https://api.viceme.ai`；`local-dev` 还要求 loopback endpoint 与 `VICEME_CLI_ALLOW_LOCAL_PROCESS_CREDENTIAL=1`。CLI 在任何请求前校验目标 origin。API 与预签名上传请求遇到重定向会直接失败，凭证请求头不会被转发到其他 origin。
...
公开 CLI 只提供一套标准认证与发布命令面。工作人员短时授权凭证只能由受信任 launcher 通过 `VICEME_ACCESS_TOKEN` 注入当前进程（`source=process`），仍只调用标准 `inspect/publish/job` 并使用统一 `x-api-key`。CLI 不提供身份选择、staff authorization 签发或凭证持久化命令；永远不输出 token，凭证生效时 login/logout fail closed，update 子进程也不会继承该凭证。
