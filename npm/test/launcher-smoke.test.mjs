...
test(
  ...
  async (context) => {
    ...
    await stat(path.join(codexHome, "skills", "viceme-checkout", "SKILL.md"));
    await stat(
      path.join(
        codexHome,
        "skills",
...
`,
    ...
    assert.equal(install.data.skills.length, 4);
    assert.equal(install.data.skills.every((skill) => skill.all_succeeded), true);
    await stat(
      path.join(isolatedEnvironment.CODEX_HOME, "skills", "viceme-checkout", "SKILL.md"),
    );
    assert.match(
