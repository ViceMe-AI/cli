...
> **Rollout status:** the Core publication transport and stable-link path are implemented, and the preview-is-the-share-link gate is enforced: after `--yes`, the publication parks at `awaiting_action` with a typed `confirm_publish` action, and the preview response carries `preview_share_url` — a hidden (creator-only) link that IS the final share link. The creator opens it, taps "Use" for at least one real successful run, then `job resume --decision confirm` flips the same link public (otherwise 409 `preview_run_required`; cancel delists the link). Natural-language candidate edits go through the `/edits` endpoint. `--yes` confirms the publication request; it is not proof that the user reviewed the final Candidate.
