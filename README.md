...
| `make-a-copy` | accept a website copy invitation; reuse account-bound or anonymous CLI purchasing when available, preserve an existing standalone recovery, and fall back to the versioned no-CLI script. |

Buyer-side `use-a-skill` and `make-a-copy` keep independent names because they are not creator workflows.
