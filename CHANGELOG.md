...
- 支持网站发布补齐创作者身份 (`6e40ac6`)
- 支持网站发布补齐创作者身份 (`602bd70`)
- 支持外部网站发布与作品售卖 (`d2f694b`)
- 增加 POC 覆盖安装与独立更新通道 (`eb793bc`)
- 限制创作者接入 Work (`3958862`)
- 优化弹幕悬浮提示交互 (`eddb32d`)
- 自动选择唯一创作者作品 (`b6caeac`)
- 提供弹幕 POC 安装入口 (`1a3c865`)
- 自动更新 CLI 与官方 Skills (`d68d096`)
- 一步创建公开弹幕接入 (`c10d4db`)
- 完善弹幕交互与页面锚点 (`13e9509`)
- add hosted danmaku sdk (`02e57c0`)
- 一步完成创作者接入 (`832aa4d`)
...
- 允许无域名发布网站作品 (`88a15dc`)
- 同步 POC Skill 发布清单 (`1738a69`)
- 规范化 POC S3 代理协议 (`8a88070`)
- 复用通用 S3 发布代理 (`66703d7`)
- 将 POC 安装器默认区域切换为 CN (`814e6eb`)
- 修正 POC 代理变量声明 (`8351683`)
- 为 POC S3 发布启用代理 (`bb3936b`)
- 修复 POC Skill 兼容性校验 (`5ef052f`)
- 修复 POC 发布恢复签名校验 (`c4d7d67`)
- 固定使用预览版 CLI 入口 (`9eb6afb`)
- 命令失败后立即停止接入 (`40778ef`)
- 默认十秒接入托管弹幕 (`1b964ff`)
- 适配新版 profile 参数 (`602b331`)
- 修正预览版兼容版本号 (`fefe411`)
- 补充网页接入登录提示 (`539fc13`)
- 统一悬浮提示样式类 (`af0a000`)
- 使用不可变 POC 下载地址 (`82b5dec`)
- 避免 POC 被正式版升级覆盖 (`df647d6`)
- 合并并发自动更新并重执行等待命令 (`f48d2f6`)

### Other Changes

- 更新 poc 清单 digest (`e57cf77`)
- 更新网站授权接入规则 (`d4a038c`)
- 更新 poc 清单 digest (`6e33350`)
- 更新 POC release manifest digest (`1e25ac7`)
- 更新 POC 安装器区域断言 (`c88c75e`)
- 兼容 Windows 工作流换行 (`0d29ee8`)
- 更新弹幕技能摘要 (`2f7732d`)
- 升级弹幕预览安装版本 (`f440c93`)
- 对齐 access 技能兼容版本 (`fec3084`)
- 更新弹幕技能发布摘要 (`26f0cbd`)
