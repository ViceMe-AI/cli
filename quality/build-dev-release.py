...
def render_dev_assets(root, build_id):
    ...
    def render(path):
        if path.suffix not in (".md", ".py", ".html", ".json", ".tmpl", ".sh", ".ps1"):
            return
        text = path.read_text()
        text = re.sub(r"https://s3\.viceme\.cn(?=/skills/|/start/|[\"\']|$)",
                      f"https://s3.dev.viceme.cn/start/builds/{build_id}", text)
        text = text.replace("https://viceme.cn", "https://dev.viceme.cn")
        # Dev is deployed only in CN. Remove GLOBAL runtime choices instead of
        # inventing an overseas dev address or falling back to production.
        if path.name in ("trial.py", "trial_runtime.py", "trial-bootstrap.py.tmpl"):
            text = re.sub(r',?\s*"global": "https://[^"\n]+",?', "", text)
        if path.name == "make_copy.py":
            text = text.replace('{"viceme.cn", "www.viceme.cn", "viceme.ai", "www.viceme.ai"}',
                                '{"dev.viceme.cn"}')
            text = text.replace('"viceme.ai" if host.endswith("viceme.ai") else "viceme.cn"',
                                '"dev.viceme.cn"')
        if path.name == "SKILL.md":
            marker = "---"
            parts = text.split(marker, 2)
            notice = ("\n> dev 测试分发：先确认 dev CLI 与 dev Profile。安装、更新、恢复由本构建的 "
                      f"https://s3.dev.viceme.cn/start/builds/{build_id}/start/agent-install.md 指导；"
...
def build(source, output, build_id, commit, platforms, allow_dirty=False):
    ...
    with tempfile.TemporaryDirectory(prefix="viceme-dev-source-") as temp:
        ...
        for item in delivery["packages"]:
            guide.append(f"| {item['os']}/{item['arch']} | [CN](https://s3.dev.viceme.cn/start/builds/{build_id}/{item['file']}) | {item['sha256']} |")
        (output / "start").mkdir()
