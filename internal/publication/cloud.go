...
func cloudManifestFromEntries(entries []sourceEntry) (*api.SkillCloudPackageManifest, error) {
	...
	if !privateSkill || len(seen) != len(files) || !exists || !publicWorkflow || !strings.EqualFold(path.Ext(manifest.LocalWorkflow), ".md") || !utf8.Valid(workflow) || len(bytes.TrimSpace(workflow)) == 0 || bytes.IndexByte(workflow, 0) >= 0 || utf8.RuneCount(workflow) > 16000 || total > 120000 {
		return invalid()
	}
