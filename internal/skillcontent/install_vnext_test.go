...
func TestInstallSetActivatesAllOfficialSkillsTogether(t *testing.T) {
	...
	writeTestSkill(t, root, "make-a-copy")
	home := t.TempDir()
	environment := Environment{Home: home, ConfigDir: filepath.Join(home, ".viceme-cli")}
	bundle := New(os.DirFS(root))

	skillNames := []string{"creator-tools", "become-a-creator", "customize-your-page", "sell-a-skill", "use-a-skill", "charge-for-your-work", "let-people-interact", "let-others-make-a-copy", "make-a-copy"}
