...
type WebsiteTutorialStep struct {
	...
	VideoLinks  []WorkVideoLink `json:"videoLinks,omitempty"`
}
...
func (content WebsiteTutorialContent) Validate() error {
	...
	for _, step := range content.Steps {
		if len(step.VideoLinks) > 20 {
			return invalid
		}
		for _, video := range step.VideoLinks {
			target, err := url.Parse(video.URL)
			if err != nil || target.Scheme != "https" || target.User != nil || !tutorialTextValid(video.Title, 1, 120, true) || !tutorialTextValid(video.URL, 1, 2048, false) || video.Type != "VIDEO_LINK" {
				return invalid
			}
			host := strings.ToLower(strings.TrimSuffix(target.Hostname(), "."))
			if !strings.Contains(host, ".") || strings.Contains(host, ":") || strings.Trim(host, "0123456789.") == "" {
				return invalid
			}
			for _, local := range []string{"localhost", "local", "internal"} {
