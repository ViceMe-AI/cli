...
type SkillCloudPackageManifest struct {
	LocalWorkflow string   `json:"localWorkflow" yaml:"localWorkflow"`
...
func (c *Client) GetCloudSkillDownload(ctx context.Context, productID string) (DownloadURL, error) {
	var result DownloadURL
	err := c.doJSON(ctx, http.MethodGet, "/v1/downloads/cloud/"+url.PathEscape(productID), nil, &result, "")
	return result, err
}
...
func (c *Client) SubmitSkillCloud(ctx context.Context, input SkillCloudSubmit, installID, secret string, kinds ...string) (SkillCloudResult, error) {
	endpoint, credential := "/v1/cli/skill-cloud/requests", "@stored"
	var body any = input
	if installID != "" {
		endpoint, credential = "/v1/skill-cloud/"+cloudCredentialKind(kinds)+"/requests", ""
		body = struct {
			SkillCloudSubmit
...
func (c *Client) ReadSkillCloud(ctx context.Context, productID, requestID, installID, secret string, resume bool, kinds ...string) (SkillCloudResult, error) {
	...
	if installID != "" {
		...
		endpoint, credential, method = "/v1/skill-cloud/"+cloudCredentialKind(kinds)+"/requests/"+url.PathEscape(requestID)+"/"+action, "", http.MethodPost
...
func cloudCredentialKind(kinds []string) string {
