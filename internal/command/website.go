...
func publishWebsite(ctx context.Context, runtime *Runtime, options websitePublishOptions) (api.SdkWork, string, error) {
	...
	if found && binding.Region != string(authority.Region) {
	...
	if found && binding.SchemaVersion == 2 &&
		(binding.APIBaseURL != authority.APIBaseURL || binding.WebBaseURL != authority.WebBaseURL) {
		return api.SdkWork{}, "", output.Policy("WEBSITE_PROFILE_MISMATCH", "website binding belongs to a different Profile authority")
	}
	if !found {
		binding = websiteBinding{
			SchemaVersion: 2, APIBaseURL: authority.APIBaseURL, WebBaseURL: authority.WebBaseURL,
			ClientWorkID: randomUUID(), Region: string(authority.Region),
		}
	} else if binding.SchemaVersion == 1 {
		if authority.APIBaseURL != config.APIBaseURL(authority.Region) || authority.WebBaseURL != config.WebBaseURL(authority.Region) {
			return api.SdkWork{}, "", output.Policy("WEBSITE_PROFILE_MISMATCH", "legacy website binding can only be migrated under its official regional Profile")
		}
		binding.SchemaVersion = 2
		binding.APIBaseURL = authority.APIBaseURL
		binding.WebBaseURL = authority.WebBaseURL
	}
	...
	if binding.WorkID != "" && (work.CreatorWorkID == nil || binding.WorkID != *work.CreatorWorkID) {
	...
	if work.CreatorWorkID == nil {
		return api.SdkWork{}, "", output.Internal("SDK_WORK_RESPONSE_INVALID", "published website response omitted creatorWorkId", nil)
	}
...
func loadWebsiteBinding(filename string) (websiteBinding, bool, error) {
	...
	if err := json.Unmarshal(data, &binding); err != nil || (binding.SchemaVersion != 1 && binding.SchemaVersion != 2) ||
		!websiteUUIDPattern.MatchString(binding.ClientWorkID) ||
		(binding.WorkKey != "" && !websiteWorkKeyPattern.MatchString(binding.WorkKey)) ||
		(binding.SchemaVersion == 2 && (binding.APIBaseURL == "" || binding.WebBaseURL == "")) {
...
func writeWebsiteBinding(filename string, binding websiteBinding) error {
	if (binding.SchemaVersion != 1 && binding.SchemaVersion != 2) || !websiteUUIDPattern.MatchString(binding.ClientWorkID) {
