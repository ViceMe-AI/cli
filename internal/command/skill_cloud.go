...
func cloudPrincipal(ctx context.Context, runtime *Runtime, input api.SkillCloudSubmit, agent string) (string, skillTrialCredential, error) {
	...
	if strings.HasPrefix(savedPrincipal, "purchase:") {
		if !purchaseExists || savedPrincipal != "purchase:"+purchase.InstallID {
			return "", credential, output.Policy("SKILL_CLOUD_REQUEST_CONFLICT", "restore the original purchase identity before retrying")
		}
	...
	if savedPrincipal == "" && !exists && !purchaseExists {
		purchase, purchaseExists, err = ensureCloudPurchaseIdentity(runtime, input.ProductID, agent)
		if err != nil {
			return "", credential, err
		}
	}
...
func runCloudTask(ctx context.Context, runtime *Runtime, input api.SkillCloudSubmit, wait time.Duration, agents ...string) (map[string]any, error) {
	...
	principal, credential, err := cloudPrincipal(ctx, runtime, input, agent)
	if err != nil {
		return nil, err
	}
	...
	result, err := runtime.client().SubmitSkillCloud(ctx, input, credential.InstallID, credential.Secret, strings.SplitN(principal, ":", 2)[0])
	if err != nil {
	...
	for (result.Status == "QUEUED" || result.Status == "RUNNING") && runtime.deps.Now().Before(deadline) {
		...
		next, err := runtime.client().ReadSkillCloud(ctx, input.ProductID, result.RequestID, credential.InstallID, credential.Secret, false, strings.SplitN(principal, ":", 2)[0])
		if err != nil {
			return nil, err
		}
