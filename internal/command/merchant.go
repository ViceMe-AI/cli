...
func newMerchantAccountsCommand(runtime *Runtime) *cobra.Command {
	return &cobra.Command{
		Use:   "accounts",
		Short: "List creator merchant accounts owned by the current login",
		Args:  cobra.NoArgs,
...
func newMerchantProductCommand(runtime *Runtime) *cobra.Command {
	...
	command.AddCommand(newMerchantBlueprintsCommand(runtime))
...
func newMerchantBlueprintsCommand(runtime *Runtime) *cobra.Command {
	var merchantAccountID string
	command := &cobra.Command{
		Use:   "blueprints",
		Short: "List blueprints available to one merchant",
		Args:  cobra.NoArgs,
		RunE: func(command *cobra.Command, _ []string) error {
			if err := runtime.requireMerchantCommerceAuthentication(command.Context(), false); err != nil {
				return err
			}
			result, err := runtime.client().ListMerchantBlueprints(command.Context(), merchantAccountID)
			if err != nil {
				return err
			}
			return runtime.business(result)
