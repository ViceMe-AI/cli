			...
			if !access.IsFree {
				...
				if !access.Owned {
					...
					if wait <= 0 && order.Status != "PAID" {
						...
						return output.Confirmation("SKILL_PURCHASE_REQUIRED", "purchase this edition before installation").WithDetails(details).WithHint(paymentPresentationHint(os.Getenv, false) + "\n" + hint)
					}
					paymentWait := wait
					if paymentWait <= 0 {
...
func installAuthorizedSkill(ctx context.Context, runtime *Runtime, productID, workSlug, agent string, access api.SkillAccess, directories ...string) (downloadableSkillInstallResult, error) {
	...
	var err error
	if access.IsFree {
		download, err = runtime.client().GetFreeSkillDownload(ctx, productID)
	} else {
		download, err = runtime.client().GetOwnedSkillDownload(ctx, productID)
	}
	if err != nil {
...
func installSkillFromReceipt(runtime *Runtime, ctx context.Context, productID, workSlug, agent string, access api.SkillAccess, download api.DownloadURL, directories ...string) (downloadableSkillInstallResult, error) {
	...
	if digest != download.ArtifactDigest {
		return downloadableSkillInstallResult{}, output.Policy("SKILL_ARTIFACT_DIGEST_MISMATCH", "downloaded Skill package does not match the active release")
	}
...
func resolveSkillUseTarget(ctx context.Context, runtime *Runtime, target string) (string, *api.PublicWorkProjection, error) {
	...
	segments, err := skillWorkURLSegments(parsed)
	if err != nil {
		return "", nil, err
	}
