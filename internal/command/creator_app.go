package command

import (
	"fmt"
	"strings"

	"github.com/ViceMe-AI/cli/internal/api"
	"github.com/ViceMe-AI/cli/internal/output"
	"github.com/spf13/cobra"
)

func newCreatorAppCommand(runtime *Runtime) *cobra.Command {
	command := &cobra.Command{Use: "creator-app", Short: "Manage Creator Apps for widget embedding"}
	command.AddCommand(newCreatorAppCreateCommand(runtime))
	command.AddCommand(newCreatorAppListCommand(runtime))
	command.AddCommand(newCreatorAppShowCommand(runtime))
	command.AddCommand(newCreatorAppDomainCommand(runtime))
	return command
}

func newCreatorAppCreateCommand(runtime *Runtime) *cobra.Command {
	var name string
	command := &cobra.Command{
		Use:   "create",
		Short: "Create an external Creator App",
		RunE: func(command *cobra.Command, args []string) error {
			if strings.TrimSpace(name) == "" {
				return output.Validation("CREATOR_APP_NAME_REQUIRED", "--name is required")
			}
			if _, err := runtime.resolveProfileAuthority(); err != nil {
				return err
			}
			app, err := runtime.client().CreateCreatorApp(command.Context(), api.CreateCreatorAppRequest{Name: strings.TrimSpace(name)})
			if err != nil {
				return creatorAppCreationError(err)
			}
			return runtime.success(map[string]any{"app": creatorAppJSON(app)})
		},
	}
	command.Flags().StringVar(&name, "name", "", "Creator App display name")
	return command
}

func newCreatorAppListCommand(runtime *Runtime) *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		Short: "List Creator Apps owned by the authenticated account",
		Args:  cobra.NoArgs,
		RunE: func(command *cobra.Command, args []string) error {
			if _, err := runtime.resolveProfileAuthority(); err != nil {
				return err
			}
			response, err := runtime.client().ListCreatorApps(command.Context())
			if err != nil {
				return err
			}
			apps := make([]any, 0, len(response.Items))
			for _, app := range response.Items {
				apps = append(apps, creatorAppJSON(app))
			}
			return runtime.success(map[string]any{"apps": apps})
		},
	}
}

func newCreatorAppShowCommand(runtime *Runtime) *cobra.Command {
	var workKey string
	var locale string
	command := &cobra.Command{
		Use:   "show <app-id>",
		Short: "Show a Creator App with its embed snippet",
		Args:  cobra.ExactArgs(1),
		RunE: func(command *cobra.Command, args []string) error {
			embedLocale, err := parseEmbedLocale(locale)
			if err != nil {
				return err
			}
			authority, err := runtime.resolveProfileAuthority()
			if err != nil {
				return err
			}
			response, err := runtime.client().ListCreatorApps(command.Context())
			if err != nil {
				return err
			}
			for _, app := range response.Items {
				if app.ID == args[0] {
					result := map[string]any{
						"app":          creatorAppJSON(app),
						"embedSnippet": buildTipEmbedSnippet(authority.WebBaseURL, app.ID, embedLocale),
					}
					workKey = strings.TrimSpace(workKey)
					if workKey != "" {
						if !accessWorkKeyPattern.MatchString(workKey) {
							return output.Validation("WORK_KEY_INVALID", "--work-key must be a public wrk_ key")
						}
						work, err := runtime.client().GetSdkWork(command.Context(), workKey)
						if err != nil {
							return err
						}
						if work.Status != "ACTIVE" || !stringSliceContains(work.Capabilities, danmakuFeatureKey) {
							return output.Policy("DANMAKU_WORK_NOT_ACTIVE", "the selected Work does not have an active public danmaku capability").
								WithHint("run 'viceme access inspect' and reconcile the Work before generating the combined embed")
						}
						result["workKey"] = work.WorkKey
						result["engagementEmbedSnippet"] = buildEngagementEmbedSnippet(
							authority.WebBaseURL, work.WorkKey, app.ID, string(authority.Region), embedLocale,
						)
					}
					return runtime.success(result)
				}
			}
			return output.Validation("CREATOR_APP_NOT_FOUND", fmt.Sprintf("Creator App %q was not found in the authenticated account", args[0]))
		},
	}
	command.Flags().StringVar(&workKey, "work-key", "", "active public danmaku Work to include in a combined engagement embed")
	command.Flags().StringVar(&locale, "locale", "zh-CN", "embed locale (zh-CN or en-US)")
	return command
}

func parseEmbedLocale(raw string) (string, error) {
	locale := strings.TrimSpace(raw)
	if locale != "zh-CN" && locale != "en-US" {
		return "", output.Validation("LOCALE_INVALID", "--locale must be zh-CN or en-US")
	}
	return locale, nil
}

func newCreatorAppDomainCommand(runtime *Runtime) *cobra.Command {
	command := &cobra.Command{Use: "domain", Short: "Manage Creator App domains"}
	command.AddCommand(newCreatorAppDomainAddCommand(runtime))
	command.AddCommand(newCreatorAppDomainVerifyCommand(runtime))
	return command
}

func newCreatorAppDomainAddCommand(runtime *Runtime) *cobra.Command {
	return &cobra.Command{
		Use:   "add <app-id> <domain>",
		Short: "Register a domain and return its verification token",
		Args:  cobra.ExactArgs(2),
		RunE: func(command *cobra.Command, args []string) error {
			if _, err := runtime.resolveProfileAuthority(); err != nil {
				return err
			}
			app, err := runtime.client().AddCreatorAppDomain(command.Context(), args[0], args[1])
			if err != nil {
				return err
			}
			return runtime.success(map[string]any{"app": creatorAppJSON(app)})
		},
	}
}

func newCreatorAppDomainVerifyCommand(runtime *Runtime) *cobra.Command {
	return &cobra.Command{
		Use:   "verify <app-id> <domain>",
		Short: "Trigger domain ownership verification",
		Args:  cobra.ExactArgs(2),
		RunE: func(command *cobra.Command, args []string) error {
			if _, err := runtime.resolveProfileAuthority(); err != nil {
				return err
			}
			app, err := runtime.client().VerifyCreatorAppDomain(command.Context(), args[0], args[1])
			if err != nil {
				return err
			}
			return runtime.success(map[string]any{"app": creatorAppJSON(app)})
		},
	}
}

func creatorAppCreationError(err error) error {
	cliError := output.AsError(err)
	unknown := cliError.Retryable
	if !unknown {
		switch cliError.Subtype {
		case "API_UNREACHABLE", "RESPONSE_READ_FAILED", "RESPONSE_TOO_LARGE", "RESPONSE_INVALID":
			unknown = true
		}
	}
	if !unknown {
		return err
	}
	recovery := output.NewError(
		output.ExitNetwork,
		"network",
		"CREATOR_APP_CREATE_OUTCOME_UNKNOWN",
		"Creator App creation outcome is unknown; do not retry create blindly",
	).WithCause(err).WithDetails(map[string]any{"causeCode": cliError.Subtype})
	recovery.RequestID = cliError.RequestID
	return recovery.WithHint("run 'viceme creator-app list' first and look for the requested app; do not retry creation until its outcome is known")
}

func creatorAppJSON(app api.CreatorApp) map[string]any {
	domains := make([]any, 0, len(app.Domains))
	for _, domain := range app.Domains {
		entry := map[string]any{
			"domain":   domain.Domain,
			"verified": domain.Verified,
		}
		if domain.VerificationToken != nil {
			entry["verificationToken"] = *domain.VerificationToken
			entry["verificationPath"] = "/.well-known/viceme-app-verification.txt"
		}
		domains = append(domains, entry)
	}
	return map[string]any{
		"id":        app.ID,
		"kind":      app.Kind,
		"name":      app.Name,
		"domains":   domains,
		"createdAt": app.CreatedAt,
	}
}

func buildTipEmbedSnippet(webBaseURL, appID, locale string) string {
	return fmt.Sprintf(
		`<script async src="%s/widget/tip-embed.js" data-creator-app-id="%s" data-locale="%s"></script>`,
		webBaseURL, appID, locale,
	)
}

func buildEngagementEmbedSnippet(webBaseURL, workKey, appID, region, locale string) string {
	return fmt.Sprintf(
		`<script defer src="%s/widget/engagement-embed.js" data-viceme-work="%s" data-creator-app-id="%s" data-viceme-region="%s" data-viceme-target="body" data-viceme-theme="auto" data-locale="%s"></script>`,
		strings.TrimRight(webBaseURL, "/"), workKey, appID, region, locale,
	)
}
