...
func performInstall(ctx context.Context, runtime *Runtime, agent, region string, ensureLauncher bool, authority *installCommitAuthority) (bootstrapInstallResult, error) {
	...
	if authenticated {
		result.NextStep = installNextStep{Command: "viceme skill publish --path <dir-or-zip>", Reason: "select the Skill Work to create or update, then upload a private Draft and open its Owner Preview"}
	} else {
