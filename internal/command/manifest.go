...
func commandPolicy(path string, runnable bool) (string, bool) {
	...
	case "skill publish", "job cancel", "job retry":
		return "public", true
	case "job resume", "job renew", "job metadata", "job edit", "job preview":
		return "public", false
	case "auth logout":
	...
	case "auth login", "config keychain-downgrade", "install", "profile add", "profile configure", "profile use", "profile remove", "profile rename", "skills install", "update":
		return "local", false
	case "version", "auth status", "profile list", "skill inspect", "skill target get", "skill target list", "job bind", "job get", "job wait", "job edit-get", "skills list", "skills read", "skills doctor":
		return "none", false
	default:
		if runnable {
