...
func commandPolicy(path string, runnable bool) (string, bool) {
	...
	case "auth login", "install", "profile add", "profile use", "profile remove", "profile rename", "skills install", "update", "skill delegated-grant save", "skill delegated-grant delete":
		return "local", false
	case "version", "auth status", "profile list", "skill inspect", "skill target get", "skill target list", "skill delegated-grant status", "job get", "job wait", "skills list", "skills read", "skills doctor":
		return "none", false
	default:
		if runnable {
